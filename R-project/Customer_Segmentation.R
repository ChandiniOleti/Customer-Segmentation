...
# Read the CSV file (using forward slashes or double backslashes in the file path)
customer_data <- read.csv("D:/R-project/Mall_Customers (1).csv")
...
#female is higher trend than male
...
boxplot(customer_data$Age,
        col=rainbow(1),
        main="Boxplot for Description Analysis of Age",labels=TRUE)
...
plot(density(customer_data$Annual.Income..k..),
     ...
     xlab="income",
     ylab="Density")
polygon(density(customer_data$Annual.Income..k..),col="brown")
...
plot(density(customer_data$Spending.Score..1.100.),
     ...
     xlab="income",
     ylab="Density")
polygon(density(customer_data$Spending.Score..1.100.),col="gray")
...
boxplot(customer_data$Spending.Score..1.100.,
        col=rainbow(1),
        main="Boxplot for Description Analysis of Spending",horizontal=TRUE)

#k-mean

#1.we specify number of clusters that we need to create
#2. The algorithms selects k objects at random from the dataset.
#This  object is the initial cluster of mean
#3.The closest centriod obtains the assignment of a new observation.we base this assignment on the
#Euclidean Distance between object and the centriod.
#update
#iteration until finish

#1.we specify number of clusters that we need to create.How?
...
#to determine and visualize the optional number of clusters
